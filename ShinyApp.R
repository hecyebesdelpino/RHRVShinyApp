...
server <- function(input, output, session) {
  ...
  message_list_value <- reactiveVal(TRUE)
  start <- reactiveVal(0)
  
#__MULTIPLE ANALYSIS__________________________________________________
        numbers <- reactive({
          validate(
            need(is.numeric(input$num_samples), "Please input a number")
          )
        })
        ...
        # })
        # 
        # # Observar el evento de clic en start
        # observeEvent(input$start, {
        #   start(1)
        #   hide(id = "start")
        # })
        ...
        # observeEvent(values$start_button, {
        #   # Actualizar el estado del botón de acción en la interfaz de usuario
        #   updateActionButton(session, "start", label = "Start", disabled = values$start_button)
        # })
        ...
        output$save_message <- renderText(
          "When the analysis finishes, do you want to save a copy of the indexes?",
        )
...
#__TIME ANALYSIS____________________________________________________________
    ...
    observeEvent(input$Analyze_Time_Button, {
      if(is.null(single_file$file_name)){
        ...
        hrv.data <- time_single_analysis(fileType = input$type_of_file, Recordname = single_file$file_name, RecordPath = single_file$path_file, 
                     size = input$window_size_button, interval = input$interval_size_button, freqhr = input$freqhr_button, method = input$frequency_method_selection)
...
#__FREQUENCY ANALYSIS______________________________________________________________
    ...
    observeEvent(input$Analyze_Fourier_Button, {
      if(is.null(single_file$file_name)){
        ...
        hrv.data = fourier_single_analysis(fileType = input$type_of_file, Recordname = single_file$file_name, RecordPath = single_file$path_file,
                     freqhr = input$freqhr_button, method = input$frequency_method_selection, size = input$window_size_button, shift = input$window_shift_button, 
                     ULFmin = input$ULFmin, ULFmax = input$ULFmax, VLFmin = input$VLFmin, VLFmax = input$VLFmax, LFmin = input$LFmin, LFmax = input$LFmax,
                     HFmin = input$HFmin, HFmax = input$HFmax, type = "fourier")
...
#__WAVELETS ANALYSIS______________________________________________________________
    ...
    observeEvent(input$Analyze_Wave_Button, {
      if(is.null(single_file$file_name)){
       ...
       hrv.data = wavelet_single_analysis(fileType = input$type_of_file, Recordname = single_file$file_name, RecordPath = single_file$path_file,
                                           freqhr = input$freqhr_button, method = input$frequency_method_selection, size = input$window_size_button,
                                           shift = input$window_shift_button, ULFmin = input$ULFmin, ULFmax = input$ULFmax, VLFmin = input$VLFmin,
                                           VLFmax = input$VLFmax, LFmin = input$LFmin, LFmax = input$LFmax, HFmin = input$HFmin, HFmax = input$HFmax,
                                           type = "wavelet", wavelet = input$wavelet_method_selection, bandtolerance = input$band_tolerance_button)
...
# resultaditos <- RHRVEasy(folders = c("C://Users/hyebe/Desktop/RHRVEasy-master/rrs/normal/", "C://Users/hyebe/Desktop/RHRVEasy-master/rrs/chf/"), size = 250, freqhr = 6, saveHRVindexesInPath = "C://Users/hyebe/Desktop/")
# save_path <- "../Desktop/RHRVEasy-master/rrs/"
# saveHRVindexes(resultaditos, save_path)

resultadito <-  RHRVEasy(folders = c("C://Users/hyebe/Desktop/RHRVEasy-master/rrs/normal/", 
                                     "C://Users/hyebe/Desktop/RHRVEasy-master/rrs/chf/",
                                     "C://Users/hyebe/Desktop/RHRVEasy-master/rrs/normal_half/", 
                                     "C://Users/hyebe/Desktop/RHRVEasy-master/rrs/chf_half/"))
